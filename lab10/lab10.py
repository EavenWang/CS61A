...
def floor_div(args):
    ...
    dividened = args.first
    p = args.rest
    while p is not nil:
        dividened = dividened // p.first
        p = p.rest
    return dividened
...
def eval_and(expressions):
    ...
    p = expressions.map(calc_eval)
    if p is nil : return True
    result = p.first
    while p is not nil:
        if p.first is scheme_f:
            return False
        result = p.first
        p = p.rest
...
def eval_define(expressions):
    ...
    bindings[key] = expressions.rest.map(calc_eval).first
    return key
